...
class PlanningGraph():
	...
	def add_action_level(self, level):
		...
		###
		#iterate over all action (self.all_actions)
		
		
		actions = set()
		for a in self.all_actions :
			a_pgnode = PgNode_a(a)
			#for each action if precond_s_nodes() in s_levels[level]
			if a_pgnode.prenodes.issubset(self.s_levels[level]) :
				#check to have one of a in the list 'actions'
					for s_node in self.s_levels[level]: #connected to the S node instances in the appropriate s_level set
						a_pgnode.parents.add(s_node)
						s_node.children.add(a_pgnode)
					#if true : append this set of actions to self.a_levels[level]
					actions.add(a_pgnode)
		#create new level of actions
	...
	def add_literal_level(self, level):
		...
		for action in self.a_levels[ level -1 ] :
			#get the effect added for each action
			for state in action.effnodes:
				#connect s as children to a
	...
	def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		for s_effect_a1 in node_a1.effnodes :
			for s_effect_a2 in node_a2.effnodes:
				if s_effect_a1.symbol == s_effect_a2.symbol and s_effect_a1.is_pos != s_effect_a2.is_pos :
					return True
		return False
	...
	def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		#compare effects of a1 with pre. of a2
		for s_effect_a1 in node_a1.effnodes :
			for s_precon_a2 in node_a2.prenodes:
				if s_effect_a1.symbol == s_precon_a2.symbol and s_effect_a1.is_pos != s_precon_a2.is_pos :
					return True
		#compare effects of a2 with pre. of a1
		for s_effect_a2 in node_a2.effnodes :
			for s_precon_a1 in node_a1.prenodes:
				if s_effect_a2.symbol == s_precon_a1.symbol and s_effect_a2.is_pos != s_precon_a1.is_pos :
					return True			
	...
	def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		for s_precon_a1 in node_a1.parents :
			for s_precon_a2 in node_a2.parents:
				#if s_precon_a1.symbol == s_precon_a2.symbol and s_precon_a1.is_pos != s_precon_a2.is_pos and 
				if s_precon_a1.is_mutex(s_precon_a2):
					return True	
		return False
	...
	def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
		...
		#if one of the parent actions are not mutex to the other parents the its not mutexes
		for action1 in node_s1.parents :
			for action2 in node_s2.parents:
				if not action1.is_mutex(action2): #a1 and a2 not mutex
					return False
		return True
	...
	def h_levelsum(self) -> int:
		...
		for goal in self.problem.goal:
			for l  in range(len(self.s_levels)):
				if PgNode_s(goal,True) in self.s_levels[l]:
					level_sum+=l
					break #break for the level states loop
